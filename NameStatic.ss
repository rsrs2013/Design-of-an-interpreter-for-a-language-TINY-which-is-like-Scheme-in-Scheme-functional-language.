...
;;---------------------------------------------------------------------
;;
;;  C A L L -- B Y -- N A M E   ,    S T A T I C    S C O P I N G
;;
;;                       Sairama Rachakonda
;;---------------------------------------------------------------------
...
(define lookup
   (lambda (env y) 
     ...
     (cond 
           ...
           ((eq? (caar env) y) (force-thunk (cadr (car env))))
...
(define delta-thunk
  (lambda (args)
    ...
    (map (lambda(a) (force-thunk a)) args)
  )
...
(define delta
  (lambda (f a)
    (let ((unthunkArgs (delta-thunk a))
          (R (lambda (s) `(&const ,s)))
          (R-1 (lambda (cl)
                 (cond
		  ((eq? (car cl) `&const) (cadr cl))
                  ((eq? (car cl) `&thunk) (force-thunk cl))
