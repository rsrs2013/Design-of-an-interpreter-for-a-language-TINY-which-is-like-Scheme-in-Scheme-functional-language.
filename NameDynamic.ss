...
(define lookup
   (lambda (env y) 
     ...
     (cond 
           ...
           ((eq? env empty-env) (error 'lookup "undefined variable,no binding exists"))
...
(define delta-thunk
  (lambda (args env)
    ...
    (map (lambda(a) (force-thunk a env)) args)
  )
...
(define delta
  (lambda (f a env)
    (let ((unthunkArgs (delta-thunk a env))
          (R (lambda (s) `(&const ,s)))
          (R-1 (lambda (cl)
                 (cond
		  ((eq? (car cl) `&const) (cadr cl))
                  ((eq? (car cl) `&thunk) (force-thunk cl env))
		  (else (error 'delta "non-constant argument"))))))
                  (R (apply (R-1 f) (map R-1 unthunkArgs))))))



(define mk-thunk                     ;; returns (&thunk env body)
  (lambda (env v)
    `(&thunk ,env ,v)))
...
(define thunkArgs
  ...
  (lambda (args env)
     ...
     (map (lambda (a) (mk-thunk env a)) args)
...
(define evalArgList* (lambda(idList argList env)
   ...
   (if (null? argList)
       env
       (evalArgList* (cdr idList) (cdr argList) (extend* env (list (car idList)) (list (mk-thunk env (car argList)))))
  )    
...
(define ev
  (lambda (e env)
    (cond
      ...
      ((eq? (car e) '&var) (let((value (lookup env (cadr e)))) (if (is-thunk? value) (force-thunk value env) value) ))          ;; e == (&var v)
      ...
      ((eq? (car e) '&let) (let* ((formalParams (cadr e))
                                 (actualArgs (caddr e))
                                 (body (cadddr e))
                                 (extEnv (extend* env formalParams (thunkArgs actualArgs env)))  )
       ...
       ((eq? (car e) '&thunk) (force-thunk e env))

      ((eq? (car e) `&apply) (let ((f (cadr e))            ;; e == (&apply f args)
				   (args (caddr e)))
			       (let ((fv (ev f env))
				     (av (map (lambda (a) (mk-thunk env a)) args)))
...
(define M1 `((lambda (x) (+ x 1)) 5))  ;; Answer: 6
(define M2 `((lambda (x) (+ x 2)) (if #t 3 2)))  ;; Answer:5
(define M3 `(lambda (x y z) ((x y) z)))          ;; Answer: '(lambda (x y z) ((x y) z))
(define M4 '((lambda (y) (y 1 2)) (lambda (x z) (+ x z))))  ;; Answer: 3
(define M5 '((lambda (x) ( (lambda (x) (+ x 1)) 2 ) ) 1))   ;; Answer: 3
(define M6 `((lambda (x z) (x 1 (z 2 2))) + *))             ;; Answer: 5
(define M7 `((lambda () 1)))                                ;; Answer: 1
(define M8 `((lambda (x) (+ x 0)) (+ 1 2)))                 ;; Answer: 3
(define M9 '((lambda (x) x) (+ 1 2)))                       ;; Answer: 3
(define M10 '((lambda (x) ((lambda (z) ((lambda (x) (z x)) 3)) (lambda (y) (+ x y)))) 1))     ;;Answer 4
(define M11 '((lambda (x) 1) ((lambda (x) (x x)) (lambda (x) (x x)))))                        ;;Answer 1
(define M20 '( (lambda (x) ((lambda (z) ((lambda (x) (+ z x)) 2)) x)) 1))                     ;;Answer 3
(define M21 '((lambda (x y) (let ((a x) (b y)) (+ a b))) 1 2))                                ;;Answer 3

(define M22 '((lambda (z) (let ((x 5))
    (let ((x 2)
          (y x))
      (list y x))))1) )                                                                       ;;Answer '(5 2)

(define M23 '((lambda (z) (let ((a 1) (b 2)) (+ a b) (* a b))) 3))

(define M24 '((lambda (z) (let* ([x z]
         [y (+ x 1)])
    (+ y x))) 3))                                                                             ;;Answer 7
